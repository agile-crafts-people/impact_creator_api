"""
E2E tests for Classification endpoints (consume-style, read-only).

These tests verify that Classification endpoints work correctly by making
...
@pytest.mark.e2e
def test_get_classifications_endpoint():
    """Test GET /api/classification endpoint."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/classification", headers=headers)
...
def test_get_classifications_with_name_filter():
    ...
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/classification?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_classification_not_found():
    """Test GET /api/classification/<id> with non-existent ID."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/classification/000000000000000000000000",
        headers=headers,
    )
...
@pytest.mark.e2e
def test_classification_endpoints_require_auth():
    """Test that classification endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/classification")
