"""
E2E tests for Post endpoints (create-style with POST and GET).
...
def test_create_post_endpoint():
    """Test POST /api/post endpoint and basic retrieval by ID and search."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-post",
        "description": "E2E test post document",
    }

    response = requests.post(f"{BASE_URL}/api/post", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-post"
